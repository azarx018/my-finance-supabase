<script lang="ts">
  ...
  import { debts as debtsStore, debtPayments, transactions } from '$lib/stores/data';
  ...
  async function removeDebt(d: SyncableRecord) {
    // Per product requirement: a debt can only be deleted once it's
    // fully settled — deleting an unpaid debt would silently erase the
    // fact that money is still owed, with no wallet-balance trace left
    // behind to explain where the cash went.
    if (!d.paid) {
      showToast('Hutang harus lunas dulu sebelum bisa dihapus', 'error');
      return;
    }
    if (!confirm(`Hapus catatan hutang "${d.name}"? Semua transaksi & cicilan terkait ikut terhapus.`)) return;

    // BUGFIX: deleting a debt used to leave its linked transactions
    // (the original borrow/lend entry AND every installment payment)
    // behind in the transaction log forever, since nothing cascaded.
    for (const t of $transactions.filter((x) => x.debt_ref === d.id)) {
      await softDeleteRecord('transactions', t.id);
    }
    for (const p of $debtPayments.filter((x) => x.debt_id === d.id)) {
      await softDeleteRecord('debt_payments', p.id);
    }
...
<div class="p-4 flex flex-col gap-4 max-w-md mx-auto">
  ...
  {#each sorted as d (d.id)}
    ...
    <div
      class="bg-base-card rounded-xl shadow-sm p-4 border"
    ...
    >
      ...
      <div class="flex gap-2 mt-3 flex-wrap">
        ...
        <button
          on:click={() => removeDebt(d)}
          disabled={!d.paid}
          title={d.paid ? 'Hapus' : 'Lunasi dulu sebelum bisa dihapus'}
          class="text-xs py-2 px-3 rounded-lg border border-border disabled:opacity-40"
          style="color: {d.paid ? 'var(--expense)' : 'var(--txt-muted)'}"
        >
          🗑️
        </button>
      </div>
