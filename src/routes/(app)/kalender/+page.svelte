<script lang="ts">
  ...
  // FAB opens the reminder sheet for whichever date is selected, falling
  // back to today if the user hasn't tapped a day yet — so "+" always
  // does something useful instead of being a no-op.
  onMount(() => fabHandler.set(() => {
    if (!selectedDate) selectedDate = todayS;
...
<div class="p-4 flex flex-col gap-4 max-w-md mx-auto">
  ...
  <div class="grid grid-cols-3 gap-2">
    <div class="bg-base-card rounded-xl shadow-sm p-2.5 border border-border text-center">
      ...
      <div class="flex flex-col gap-2">
        ...
        {#each dayTxs as t (t.id)}
          ...
          <div class="flex items-center gap-3 bg-base-card rounded-xl shadow-sm p-3 border border-border">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm text-txt-primary truncate">{t.description || 'Transaksi'}</p>
