...
<div class="p-4 flex flex-col gap-3 max-w-md mx-auto">
  ...
  {#each list as t (t.id)}
    ...
    {#if isLinked}
      <div class="flex items-center gap-3 bg-base-card rounded-lg p-3 border border-border text-left">
    ...
    {:else}
      <button
        on:click={() => openEdit(t)}
        class="flex items-center gap-3 bg-base-card rounded-lg p-3 border border-border text-left"
      >
        <span class="text-xl">{cat?.emoji ?? '💸'}</span>
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-txt-primary truncate">{t.desc}</p>
