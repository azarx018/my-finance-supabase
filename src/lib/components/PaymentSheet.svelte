<script lang="ts">
  ...
  async function submit() {
    ...
    if (!walletId) {
      // BUGFIX: this used to be missing entirely, so a payment could be
      // submitted with wallet_id = '' — the debt_transfer transaction
      // still got created, but computeWalletStats() only touches a
      // wallet's balance when it can match the transaction's wallet_id
      // to a real wallet. An empty wallet_id matched nothing, so net
      // worth never moved even though the debt showed as paid.
...
<BottomSheet {open} {onClose}>
  {#if debt}
    ...
    <div class="bg-base-card2 rounded-xl shadow-sm p-3 mb-4">
